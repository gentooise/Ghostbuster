...
Furthermore, we make the same assumptions contained in the threat model of \mysec{sec:threat_model}.
...
\begin{itemize}
	...
	\item \itemname{Debug subsystem}: the SoC subsystem that enables debug capabilities for operating system and processes.
...
\begin{itemize}
	...
	\item \itemname{raising the bar}: they limit the attacker possibilities, by restricting access to the corresponding subsystem;
	\item \itemname{forensics}: they provide additional information, useful to figure out how an eventual attack has been conducted.
\end{itemize}
...
before getting noticed. A smaller interval, instead, may cause too much performance overhead. We provide our experimental results in \mychap{chap:results}.
...
\end{itemize}
See the implementation part in \mysec{sec:def_impl} for more details.
